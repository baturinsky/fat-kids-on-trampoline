...
function gameUpdated(g: Game) {
  ...
  if(game){
    let tip = 
`Neighborhood kids caught a wind of you getting a giant trampoline and want to play.<br/>
...
`;
    let text = game.over()?`<h3>Game over</h3>You are overrun.<br/><br/>Press ESC to restart`:paused?`<h3>Paused</h3></br>${tip}`:game.playersAlive()?"":tip;
    ui.innerHTML = text;
  }
}
